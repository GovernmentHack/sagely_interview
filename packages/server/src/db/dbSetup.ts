...
export async function connectToDatabase() {
  if (!process.env.MONGODB_URI || !process.env.DB_NAME) {
    throw new Error(
      "No connection string or db name provided, cannot connect to db"
    );
  }
  ...
  const publicationsCollection: mongoDB.Collection =
  ...
  console.log(
    `Successfully connected to database: ${db.databaseName} and collection: ${publicationsCollection.collectionName}`
  );
}
